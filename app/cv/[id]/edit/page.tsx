  ...
  */
  ...
  const callCvAi = useCallback(async (payload: Record<string, unknown>) => {
    ...
    if (!response.ok) {
      ...
      if ((response.status === 402 || response.status === 403) && result?.upgradeRequired) {
        ...
        const error = new Error(result.message || 'Bạn cần nâng cấp Premium hoặc nạp thêm credit để sử dụng tính năng này.') as Error & { handled?: boolean };
        error.handled = true;
        throw error;
      }
  ...
  const handleRunStudentAi = useCallback(async (action: StudentAiAction) => {
    ...
    if (action === 'fresherSummary' && !canGenerateFresherSummary) {
      if (aiPlan === 'free' && (remainingToday ?? 0) <= 0) {
        openQuotaPrompt('Bạn đã dùng hết lượt AI miễn phí hôm nay. Nâng cấp Premium để tiếp tục dùng fresher summary.');
      } else {
        toast.error('Không thể dùng Fresher Summary lúc này.');
      }
      return;
    }

    if (action === 'generateProjectBullets' && !canGenerateProjectBullets) {
      handleAiRequestError(new Error('Project Bullets hiện chỉ có trong gói Premium.'), 'Tính năng này cần gói Premium.');
      return;
    }

    if (action === 'convertActivitiesToCvBullets' && !canConvertActivitiesToCvBullets) {
      handleAiRequestError(new Error('Activities to CV Bullets hiện chỉ có trong gói Premium.'), 'Tính năng này cần gói Premium.');
      return;
    }
    ...
    if (action !== 'fresherSummary' && !studentAiForm.itemName.trim()) {
      toast.error('Hãy nhập tên đồ án hoặc hoạt động trước khi dùng AI.');
      return;
    }
    ...
    try {
      ...
      if (!newText.trim()) {
        throw new Error('AI chưa trả về nội dung phù hợp để áp dụng.');
      }
      ...
      toast.success('AI đã tạo bản đề xuất. Hãy xem lại trước khi áp dụng.');
    } catch (error) {
      ...
      if (
        ...
        toast.error(error instanceof Error ? error.message : 'AI trả về định dạng không hợp lệ.');
        return;
      }

      toast.error(error instanceof Error ? error.message : 'Không thể tạo nội dung AI cho sinh viên/Fresher.');
    } finally {
  ...
  const handleApplyPendingStudentAi = useCallback(() => {
    ...
    setPendingStudentAi(null);
    toast.success('Đã áp dụng nội dung AI vào CV.');
  ...
  const handleDownloadCoverLetter = useCallback(() => {
    if (!coverLetterDraft.trim() || !cv) {
      toast.error('Chưa có cover letter để tải xuống.');
      return;
    }
    ...
    toast.success('Đã tải xuống cover letter.');
  }, [coverLetterDraft, cv]);
  ...
  const handleApplyAtsSuggestions = useCallback(() => {
    ...
    if (!atsReview.suggestedSummary.trim() && atsReview.suggestedExperienceBullets.length === 0) {
      toast.error('Chưa có gợi ý ATS để áp dụng.');
      return;
    }
    ...
    toast.success('Đã áp dụng gợi ý ATS vào CV hiện tại.');
                          ...
                          <div>
                            ...
                            <div style={{ marginBottom: '12px', borderRadius: '10px', border: `1px solid ${canRunAtsReview ? 'rgba(245,158,11,0.25)' : 'rgba(148,163,184,0.2)'}`, background: canRunAtsReview ? 'rgba(245,158,11,0.06)' : 'rgba(148,163,184,0.05)', padding: '12px' }}>
                              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', marginBottom: atsReview ? '12px' : 0 }}>
                                <div>
                                  <p style={{ fontWeight: 700, fontSize: '0.84rem', color: 'var(--text-primary)', marginBottom: '4px' }}>
                                    Tối ưu theo JD
                                  </p>
                                  <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                                    {canRunAtsReview
                                      ? 'Phân tích CV theo vị trí, công ty và JD hiện tại để chấm điểm match, tìm keyword thiếu và tạo gợi ý áp dụng nhanh.'
                                      : 'Tính năng này cần Premium hoặc đủ credit. Bạn vẫn có thể chuẩn bị JD và mở khóa khi cần.'}
                                  </p>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                                  {!canRunAtsReview && (
                                  ...
                                  <button type="button" onClick={handleAtsReview} disabled={!canRunAtsReview || atsLoading} className="btn btn-outline btn-sm">
                                    {aiStatusLoading ? 'Đang kiểm tra quyền...' : atsLoading ? 'Đang phân tích...' : 'Phân tích CV'}
                                  </button>
                                </div>
                              </div>
                              {atsReview && (
                            ...
                            <div style={{ marginBottom: '12px', borderRadius: '10px', border: `1px solid ${canGenerateCoverLetter ? 'rgba(236,72,153,0.22)' : 'rgba(148,163,184,0.2)'}`, background: canGenerateCoverLetter ? 'rgba(236,72,153,0.05)' : 'rgba(148,163,184,0.05)', padding: '12px' }}>
                              ...
                              {!canGenerateCoverLetter && (
                                <div style={{ marginBottom: '12px' }}>
                                  <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                                    Cover letter generator cần Premium hoặc đủ credit. Khi mở khóa, bạn có thể tạo thư ứng tuyển bám sát JD và lưu vào tài khoản.
                                  </p>

...
const TRANSFORMATIONS: Transformation[] = [
  {
    ...
    tip: 'AI tự động bổ sung kết quả và từ khóa tuyển dụng',
  },
  {
    category: 'Mục tiêu nghề nghiệp',
    before: 'Tìm việc làm IT, muốn học hỏi và phát triển bản thân.',
    after: 'Kỹ sư phần mềm với 1 năm kinh nghiệm phát triển ứng dụng web, tìm kiếm cơ hội tại môi trường công nghệ năng động để đóng góp vào các dự án thực tiễn và phát triển chuyên môn trong lĩnh vực frontend hoặc full-stack.',
  ...
  },
  {
    ...
    after: 'Phát triển ứng dụng quản lý sinh viên toàn diện sử dụng React.js và Node.js, tích hợp cơ sở dữ liệu MySQL. Hệ thống hỗ trợ quản lý hơn 500 hồ sơ sinh viên, giảm 40% thời gian xử lý thủ công so với phương pháp cũ.',
    tip: 'AI thêm số liệu cụ thể và từ khóa kỹ thuật phù hợp',
  },
  {
    category: 'Kỹ năng và thành tích',
    before: 'Giỏi teamwork, chăm chỉ, học nhanh.',
    after: 'Kỹ năng làm việc nhóm hiệu quả trong môi trường Agile hoặc Scrum. Nhanh tiếp thu công nghệ mới, hoàn thành 3 dự án cá nhân trong 6 tháng và tham gia 2 hackathon với kết quả Top 10.',
...
export default function BeforeAfterShowcase() {
  ...
  const handleNext = () => {
    setShowAfter(false);
    setTimeout(() => setActiveIdx(index => (index + 1) % TRANSFORMATIONS.length), 200);
  };
              ...
              {TRANSFORMATIONS.map((item, index) => (
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    textAlign: 'left',
                    background: index === activeIdx ? 'rgba(139,92,246,0.1)' : 'var(--bg-card)',
                    border: `1px solid ${index === activeIdx ? 'rgba(139,92,246,0.4)' : 'var(--border)'}`,
                    transition: 'all 0.2s',
                    fontFamily: 'inherit',
                    color: index === activeIdx ? '#7c3aed' : 'var(--text-secondary)',
                    fontWeight: index === activeIdx ? 700 : 500,
                  }}
                >
                  <div
                    style={{
                      ...
                      borderRadius: '50%',
                      flexShrink: 0,
                      background: index === activeIdx ? 'linear-gradient(135deg, #8b5cf6, #6366f1)' : 'var(--border)',
                      transition: 'all 0.2s',
                    }}
                  />
                  <span style={{ fontSize: '0.88rem' }}>{item.category}</span>
                  {index === activeIdx && <ArrowRight size={14} style={{ marginLeft: 'auto' }} />}
                </button>
              ))}
            </div>
                ...
                <div style={{ textAlign: 'center', margin: '0 0 12px' }}>
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: '12px',
                      cursor: 'pointer',
                      background: showAfter ? 'rgba(16,185,129,0.1)' : 'linear-gradient(135deg, #8b5cf6, #6366f1)',
